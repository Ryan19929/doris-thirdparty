...
void testCharacterUtil(CuTest* tc) {
    ...
    CharacterUtil::processString(text, strlen(text), typedRuneArray, true);
...
void testCJKSegmenter(CuTest* tc) {
    auto cfg = std::make_shared<Configuration>("./ik-dict");
    auto context = std::make_shared<AnalyzeContext>(cfg);
    CJKSegmenter segmenter;
    IKArbitrator arbitrator;
    const char* testStr1 = "我";
    auto reader1 =
    ...
    const char* testStr2 = "研究生";
    context->reset();
    auto reader2 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr2, strlen(testStr2), false);
    ...
    context->reset();
    auto reader3 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr3, strlen(testStr3), false);
    ...
    const char* testStr4 = "我的研究生同学";
    context->reset();
    auto reader4 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr4, strlen(testStr4), false);
...
void testLetterSegmenter(CuTest* tc) {
    auto cfg = std::make_shared<Configuration>("./ik-dict");
    std::shared_ptr<AnalyzeContext> context = std::make_shared<AnalyzeContext>(cfg);
    LetterSegmenter segmenter;
    IKArbitrator arbitrator;

    const char* testStr1 = "B";
    auto reader1 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr1, strlen(testStr1), false);
    context->fillBuffer(reader1.get());
    ...
    context->reset();
    auto reader5 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr5, strlen(testStr5), false);
...
void testCNQuantifierSegmenter(CuTest* tc) {
    auto cfg = std::make_shared<Configuration>("./ik-dict");
    std::shared_ptr<AnalyzeContext> context = std::make_shared<AnalyzeContext>(cfg);
    CN_QuantifierSegmenter segmenter;
    IKArbitrator arbitrator;

    const char* testStr2 = "三个苹果";
    context->reset();
    auto reader2 =
            std::make_unique<lucene::util::SStringReader<char>>(testStr2, strlen(testStr2), false);
    context->fillBuffer(reader2.get());
