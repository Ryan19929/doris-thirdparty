...
int32_t CharacterUtil::identifyCharType(uint32_t rune) {
    // Numbers
...
uint32_t CharacterUtil::regularize(uint32_t rune, bool use_lowercase) {
    ...
    // Full-width letters
    if (rune >= 0xFF21 && rune <= 0xFF3A) { // Full-width uppercase letters
        rune = rune - 0xFEE0;               // Convert to half-width
        if (use_lowercase) {
            rune += 32; // Convert to lowercase
        }
        return rune;
    }
    if (rune >= 0xFF41 && rune <= 0xFF5A) { // Full-width lowercase letters
        return rune - 0xFEE0;               // Convert to half-width
    }
...
void CharacterUtil::processString(const char* str, size_t length, TypedRuneArray& typed_runes,
                                  bool use_lowercase) {
    typed_runes.clear();
    size_t byte_pos = 0;
    ...
    while (byte_pos < length) {
        ...
        TypedRune typedRune;
        RuneStr tempRune(runeStr.rune, byte_pos, runeStr.len, typed_runes.size(), 1);
        typedRune.init(tempRune);

        if (use_lowercase) {
            typedRune.regularize(true);
        }

        typed_runes.push_back(typedRune);
