...
public:
    ...
    public:
        ...
        uint32_t getChar() const { return rune; }
    ...
    static int32_t identifyCharType(uint32_t rune);

    static void processString(const char* str, size_t length, TypedRuneArray& typed_runes,
                              bool use_lowercase);

    static uint32_t regularize(uint32_t rune, bool use_lowercase);
